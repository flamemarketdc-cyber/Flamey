...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ session, onLogout, server, onGoToServerSelector, onServerSelected, onLogin, onToggleSidebar }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 w-full h-16 bg-nexus-overlay/80 backdrop-blur-sm border-b border-white/5 z-30 flex items-center justify-between px-8">
        {/* Left Side */}
...
const Dashboard: React.FC<DashboardProps> = ({ session, onLogout, server, onGoToServerSelector, onServerSelected, onLogin }) => {
    ...
    useEffect(() => {
        const handleHashChange = () => {
            const newFeature = getFeatureFromHash();
            if (hasUnsavedChanges && newFeature !== activeFeature) {
                // Prevent navigation and shake the save bar
                window.history.replaceState(null, '', `#/dashboard/${activeFeature}`);
    ...
    const handleSetActiveFeature = (feature: Feature) => {
        if (hasUnsavedChanges && feature !== activeFeature) {
            setShakeKey(key => key + 1);
        } else {
